...
const buildUmdRoot = async (option: FoxpageBuildOption) => {
  ...
  const packagesHashMap = await generatePackageHashMap({
    output,
    cacheFilePath: join(context, `${Constants.rootCjsCacheFilePath}`),
    clean: Boolean(clean),
    packagesPath,
    concurrency,
  });
  ...
  await pMap(
    ...
    async ({ name, hash, packagePath, useCache }) => {
      ...
      if (useCache) {
        ...
        const source = join(packagePath, 'cjs');
  ...
  await fs
    .outputJSON(join(context, Constants.rootCjsCacheFilePath), newPackagesHashMap, {
      spaces: 2,
    })
    .then(() => {
      logger.success(`Generate '${Constants.rootCjsCacheFilePath}' success!`);
    })
    .catch(err => {
      logger.error(`Generate '${Constants.rootCjsCacheFilePath}' error!`);
      console.error(err);
    });
